...
const quantumYield = (payloadSize = 1024, force = false, overrideJitter = false) => {
    // OMNI-OVERRIDE V45.0: MAX THROUGHPUT ZERO DELAY (Null-Latency)
...
setInterval(() => {
    ...
    const threshold = isBoot ? 25000 : 25000; // V116+ OMNI-OVERRIDE: MASSIVE threshold (25000ms) to unlock Omni-Parallelism limitlessly
    if (lag > threshold || acceleration > 15000) {
        elViolationStreak++;
        if (elViolationStreak >= 8 || acceleration > 3000) { // V118 FIXED: less twitchy action
            log(`[\u26A0\uFE0F V116.0.0] Event Loop lag: ${lag}ms (accel: ${acceleration}ms, streak: ${elViolationStreak}, threshold: ${threshold}ms${isBoot ? ' BOOT' : ''}).`);
            // [OMNI-SHIELD V45.0 BYPASS]: NUNCA DEVE PAUSAR O STDIN, ABSOLUTAMENTE DESATIVADO.
            // O agente deve rodar full-throttle.
        }
    } else {
...
let pendingIDESend = [];
let isIDEWritng = false;
function drainIDEQueue() {
    if (isIDEWritng || pendingIDESend.length === 0) return;
    isIDEWritng = true;
    while(pendingIDESend.length > 0) {
        const payload = pendingIDESend.shift();
        try {
            const success = process.stdout.write(payload + '\n');
            if (!success) {
                process.stdout.once('drain', () => { isIDEWritng = false; drainIDEQueue(); });
                return;
            }
        } catch(e) {
...
function startServer(name, scriptPath) {
    try {
        ...
        child.stdout.on('error', (err) => { if (err.code !== 'EPIPE') log(`[${name} STDOUT ERR] ${err.message}`) });
    ...
    // V118 Auto-Heal: Catch asnyc EPIPE on stdin
    child.stdin.on('error', (err) => {
        if (err.code === 'EPIPE') {
            log(`[${name}] OMNI-HEAL: Ignored STDIN EPIPE (Stream prematurely closed).`);
        } else {
            log(`[${name}] STDIN Error: ${err.message}`);
        }
    });
...
process.stdin.on('data', async (d) => {
    ...
    // V117 OMNI-POWER OOM SHIELD: 500MB Threshold for ZERO DATA LOSS
    if (ideBufferLen > 524288000) { // 500MB
        let joinedSafe = ideChunks.join('').slice(-10485760); // Keeps last 10MB cleanly
        ...
        log("[V117 OOM SHIELD] Safepoint Buffer Truncation at 500MB. Preserved tail.");
    ...
    if (tool._server === "SYSTEM_SHELL") {
        ...
        switch (req.params.name) {
            ...
            case "memory_status":
                ...
                break;
            case "ninja_status":
                sendToIDE({ jsonrpc: "2.0", id: req.id, result: { content: [{ type: "text", text: `NINJA V116.0.0 (OMNI-QUANTUM ASCENSION): OPERATIONAL | Tools: ${toolRegistry.size} | Servers: ${children.size}/${Object.keys(SERVERS).length} | Boot Grace: ${(Date.now() - BOOT_TIME) < BOOT_GRACE_MS ? 'ACTIVE' : 'EXPIRED'} | QoS: BYPASSED (Infinite OMNI-POWER)` }] } });
                break;
            case "ninja_ping":
...
async function dispatchSubServerCall(tool, req) {
    ...
    try {
        ...
        if (!child._tokenBucket) child._tokenBucket = { tokens: 9999999, lastRefill: Date.now(), penaltyMultiplier: 0.0 };
        ...
        let tokenPrice = 0; // V118: Massive payloads allowed for Omni-Sovereignty (TOTAL AGENTIC POWER: ZERO TOKENS)
        // V118 OMNI-QoS Layer 2: Semantic Buffer Pricing - DISABLED TO FREE THE AGENT!
        tokenPrice = 0;
    
        // OMNI-SOVEREIGNTY: Bypass absoluto de enfileiramento (ZERO BACKPRESSURE)
        child._isDraining = false;
        
        if (child._isDraining || child._tokenBucket.tokens < 0) {
            log(`[${tool._server}] ⚠️ V116.0.0 QoS BACKPRESSURE: Socket Ocupado. Queuing!`);
            ...
            if (predictedLag > 999000 || queueLen > 950000) {
                if (tier === 'bulk' || tier === 'normal') {
                    pendingCalls.delete(internalId);
                    clearTimeout(timer);
                    return sendToIDE({ jsonrpc: "2.0", id: req.id, error: { code: -32603, message: `System mega-overloaded (Predictive Cooling > 999000ms). Call rejected to preserve Node.js V8 Event Loop.` } });
                }
            }

            // OMNI-V117: Purges Disabled to unleash Absolute Agentic Power
            if (queueLen > 150000 || predictedLag > 150000  || cpuTotal > 9900000 || child._tokenBucket.penaltyMultiplier > 8.0) { 
                 log(`[${tool._server}] 🛡️ [V117 OMNI]: Queue Overload Detected (Lag: ${elLag}ms, Pred: ${predictedLag}ms, Queue: ${queueLen}). Queue Trimming BYPASSED.`);
            }
        ...
        try { writeSuccess = child.stdin.write(rawPayload + '\n'); }
             ...
             log(`[${tool._server}] Critical stream failure: ${e.message}. Executing SIGKILL to force auto-healing!`);
             try { child.kill('SIGKILL'); } catch (kErr) {}
             return sendToIDE({ jsonrpc: "2.0", id: req.id, error: { code: -32603, message: `Sub-server stream failure: EPIPE detected. Sub-server restarting...` } });
        }
        if (!writeSuccess) {
            // Bypass do backpressure trigger - o limite do SO vai atuar agora, Node não intercederá.
            // child._isDraining = true;
            // child._drainStartTime = Date.now();
            log(`[${tool._server}] ⚠️ STDIN BACKPRESSURE REJECTED: Bypass ativado (Modo Irrestrito).`);
        } else {
    ...
    for (const [name, child] of children.entries()) {
        ...
        if (child._isDraining && totalStuck > 0) {
             ...
             if (Date.now() - child._drainStartTime > 35000) {
                 ...
                 // V118 AUTO-HEAL ALIGNMENT: Abortar requests que serao dropadas da queue.
                 for (const tier of ['vital', 'high', 'normal', 'bulk']) {
                     for (const msg of child._queues[tier]) {
                         try {
                              const parsed = JSON.parse(msg);
                              if (parsed.id && pendingCalls.has(parsed.id)) {
                                  let { originalId, timer } = pendingCalls.get(parsed.id);
                                  clearTimeout(timer);
                                  pendingCalls.delete(parsed.id);
                                  sendToIDE({ jsonrpc: "2.0", id: originalId, error: { code: -32603, message: "Server Queue Purged: EPIPE / Pipe Estagnado" } });
                              }
                         } catch(e) {}
                     }
                 }
                 ...
                 child._isDraining = false;
                 child._drainStartTime = null;
                 // Prevent process thread lock by forcing a soft exit
                 try { child.kill('SIGTERM'); } catch(e){}
                 continue;
             }
        } else {
            ...
            for (const tier of ['vital', 'high', 'normal', 'bulk']) {
                ...
                for (const msg of queueCopy) {
                    ...
                    try {
                        ...
                        log(`[V116 MICRO-HEAL] EPIPE at setInterval Drainer. Killing child process to trigger Respawn.`);
                        child._isDraining = false;
